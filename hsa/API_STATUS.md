...
LSE's pinned HRX resolves **119/119 symbols**, and the runtime supports validated
R9700 Qwen inference through HRX/Loom. Symbol coverage does not imply full HSA
conformance: platform-unsupported APIs deliberately return errors, and several
families below support host behavior only.
...
profiling can now be enabled before the first submission. Used queues reject
property changes; raw GPU dispatch timestamps are validated through
[rocprofmac](../tools/rocprofmac/README.md). Host-correlated standard profiling
queries and hardware counters remain unavailable.
...
suites. Actual HRX GPU compute and LSE model inference pass the
[documented hardware checks](../docs/LSE_PERFORMANCE.md). Symbol resolution or
successful host compilation alone does not establish hardware correctness.
...
| Queue profiling/info/CU-mask/priority | 4 | Profiling can be enabled on an unused hardware queue; changes after submission are rejected. Raw CP timestamp capture passes on driver 195. Persistent queues return their owning agent. Physical doorbell IDs, CU affinity and priority changes remain unsupported. |
...
General global linking remains incomplete. Scratch/LDS growth and reuse have
passed hardware output and guard validation.
