...
"$llvm_bin/clang++" -std=c++26 -Wall -Wextra -Werror -I"$lse_source/include" \
  hsa/tools/mac_lse_pointwise_smoke.cpp \
  ...
  -o "$lse_build/mac-lse-pointwise-smoke"
printf 'Built pointwise smoke; no GPU calls run. CPU check:\n%q --check-reference\n' "$lse_build/mac-lse-pointwise-smoke"
printf 'Explicit GPU command:\nDYLD_LIBRARY_PATH=%q %q --run\n' "$PWD/build/hsa" "$lse_build/mac-lse-pointwise-smoke"
