...
"$llvm_bin/clang++" -std=c++26 -Wall -Wextra -Werror -I"$lse_source/include" \
  hsa/tools/mac_lse_decode_attention.cpp \
  ...
  -o "$lse_build/mac-lse-decode-attention"
printf 'Built LSE decode attention qualification tool; no GPU calls run. Explicit command:\n'
printf 'DYLD_LIBRARY_PATH=%q %q --run\n' "$PWD/build/hsa" "$lse_build/mac-lse-decode-attention"
printf 'Compile-only validation (no GPU):\n'
printf '%q --compile-only %q\n' "$lse_build/mac-lse-decode-attention" "$PWD/build/tests/decode-attention"
printf 'Optional qualification filters: --mask-mode causal|none; timing: --benchmark\n'
