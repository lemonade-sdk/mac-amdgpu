...
"$llvm_bin/clang++" -std=c++26 -Wall -Wextra -Werror -I"$lse_source/include" \
  hsa/tools/mac_lse_paged_smoke.cpp \
  ...
  -o "$lse_build/mac-lse-paged-smoke"
printf 'Built LSE paged KV/attention GPU smoke; no GPU calls run. Explicit command:\n'
printf 'DYLD_LIBRARY_PATH=%q %q --run\n' "$PWD/build/hsa" "$lse_build/mac-lse-paged-smoke"
