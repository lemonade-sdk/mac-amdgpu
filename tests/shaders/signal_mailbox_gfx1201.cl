#include "../../hsa/src/signal_mailbox_layout.h"
...
__kernel void signal_mailbox(__global ulong *mailbox,__global ulong *signal,
                            ulong batches,ulong max_attempts) {
    if (__builtin_amdgcn_workitem_id_x()) return;
    STORE(mailbox+MAC_MAILBOX_READY,1UL);
    for (ulong sequence=1;sequence<=batches;++sequence) {
        ulong attempts=0;
        while (LOAD(mailbox+MAC_MAILBOX_REQUEST_SEQUENCE)!=sequence) {
            if (++attempts>=max_attempts || LOAD(mailbox+MAC_MAILBOX_ABORT)) goto cancelled;
        }
        if (LOAD(mailbox+MAC_MAILBOX_ABORT)) goto cancelled;
