...
inline bool aql_scratch_geometry(uint32_t laneBytes, uint32_t cuCount,
    ...
    if (wavesPerEngine>0xfff) return false;
    bytes=uint64_t(alignedLaneBytes)*64*wavesPerEngine*engines;
    return true;
}
inline bool aql_scratch_metadata(amd_queue_t &metadata, uint64_t base,
    uint64_t bytes, uint32_t laneBytes, uint32_t engines, uint32_t wavesPerEngine) {
    if (!base || (base&16383) || base>=(1ull<<48) || !bytes || bytes>UINT32_MAX ||
        bytes>(1ull<<48)-base || !laneBytes || (laneBytes&15) ||
        laneBytes>kAQLMaxPrivateBytes || !engines || engines>4 ||
        wavesPerEngine<kAQLScratchMinimumWavesPerEngine || wavesPerEngine>0xfff ||
    ...
    // gfx12 WAVES [11:0], WAVESIZE [29:12], in KiB per 64-lane wave.
    metadata.compute_tmpring_size=wavesPerEngine|((laneBytes/16)<<12);
    return true;
}
}
